...
- [ ] 任务 10：公开论文《中国A股市场动量效应的特征和形成机理研究》（财经研究，2014）已完成固定版本运行 `run_e66408a5fdf4`。原 20 标的、原始不复权的 5/10/15 日运行已由 `run_af74f92dd2fa`、`run_bad281ab64cb`、`run_5f876bf9d8f4` 替代；三次均使用 `v20260724_de5cde1b`、`warehouse`、`fallback_used=false`、未设股票池上限的 2,544 标的、公司行为总回报价（相关事件 17,111，应用 16,730）和与持有期一致的非重叠信号日（432/215/142）。`momentum_20` 均因负 IC、ICIR、最大回撤和 Walk-forward 门禁被拒绝，未进入实验、因子库、PM 建议或人工决策。第二个独立、CC BY 的 PLOS ONE 来源 Shi, Jiang and Zhou (2015) 已通过保存的来源证据和预注册 DSL `rank(-returns(close, 20))` 在同一版本复验：`run_ef35692c11f6` 使用 20 日形成/20 日持有、107 个非重叠信号、2,544 标的和相同公司行为口径，IS/OOS Rank IC 为 `0.067918`/`0.086893`，ICIR 为 `0.447179`/`0.501778`，Walk-forward 5/5 正向，故进入候选 `factor_8e5f9ea5446c4b4f94cca2204312d5ea`。PM 建议因净多头最大回撤 `-51.8042%` 为 `reject`；无人工决定，候选仍为 `candidate`，且为避免事后挖掘未开启变形实验。英文 `contrarian` 现在会由确定性规则抽取为 `contrarian_loser_20` 并生成 `rank(-returns(close, 20))`；预置规格运行的报告改用原始来源而非抽取回退来源，现有 run 的报告和 bundle 已以 `artifact_amended` 事件修订，未改动市场数据或指标。
