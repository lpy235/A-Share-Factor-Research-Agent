...
- [ ] 阶段 1：版本化 A 股原始日频数据仓；已完成版本目录、Parquet 表存取、未复权数据源适配、可恢复采集、质量发布联动、父子版本链读取、CSV 运维入口、研究任务固定绑定已发布数据版本、基线验收运行手册、公开候选源决策记录及可审计本地 CSV 发布命令；待取得生产数据快照、补齐更细质量规则并完成真实数据演练。
